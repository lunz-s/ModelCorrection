...
# Including forward an backward projections as suggested in Hauptmann et al, MLMIR 2018 
''' Latest version: 10.10.2018, Andreas Hauptmann '''
...
#        pT=np.transpose(pT,(1,0))        
        ...
        ky_new= np.real( np.sign(w)*np.sqrt( np.square(w/c) - np.square( self.kgridBack.ky) )    )

        max_angle = self.angThresh
        min_ky = np.abs( (w/c) * np.cos( np.deg2rad(max_angle) ) )
        wf = w / (c * ky_new)
        idx  = np.where(np.isnan(wf))
        wf[idx]=0
        idx  = np.where(np.abs(ky_new) < min_ky)
