...
class ClassicalCorrection(model_correction):
    ...
    # def log_gt_optimization(self, image, recursions, step_size, lam, positivity=True, verbose=False, tensorboard=True, n_print=5):
    #     x, true = self.sess.run([self.x_ini, self.measurement], feed_dict={self.input_image: image})
    #     if tensorboard:
    #         writer = tf.summary.FileWriter(self.raw_path + 'GradDesc/Lambda_{}/GroundTruth'.format(lam))
    #     for k in range(recursions):
    #         if verbose and k % n_print == 0:
    #             summary, data_grad, TV_grad, quality, dataTerm, angle, uncor_angle, \
    #             true_grad, approx_grad, recon, uncor_grad = self.sess.run(
    #                 [self.merged_opt, self.approx_grad, self.TV_grad, self.quality, self.DataTermTrue, self.angle_true,
    #                  self.angle_uncorrected, self.true_grad, self.approx_grad, self.input_image, self.uncorrected_grad],
    #                                feed_dict={self.input_image: x, self.data_term: true,
    #                                           self.ground_truth: image})
    #             print(f'Quality {quality}, Data Term {dataTerm}, Angle {angle}, Uncor Angle {uncor_angle}')
    #             # Plotting reconstruction
    #             plt.figure(figsize=(18,4))
    #             plt.subplot(141)
    #             plt.imshow(true_grad[0,...,0])
    ...
    #
    #         else:
    #             summary, true_grad, TV_grad = self.sess.run([self.merged_opt, self.true_grad, self.TV_grad],
    #                                feed_dict={self.input_image: x, self.data_term: true,
    #                                           self.ground_truth: image})
    #         if tensorboard:
    #             writer.add_summary(summary, k)
    #         x = self.update(x, true_grad, TV_grad, lam=lam, step_size=step_size, positivity=positivity)
    #     if tensorboard:
    #         writer.flush()
    #         writer.close()
    #
    # def log_approx_optimization(self, image, recursions, step_size, lam, positivity=True, verbose=False, tensorboard=True, n_print=5):
    #     x, true = self.sess.run([self.x_ini, self.measurement], feed_dict={self.input_image: image})
    #     if tensorboard:
    #         writer = tf.summary.FileWriter(self.raw_path + 'GradDesc/Lambda_{}/ApproxUncorrected'.format(lam))
    #     for k in range(recursions):
    ...
    #
    #         else:
    #             summary, uncor_grad, TV_grad = self.sess.run([self.merged_opt, self.uncorrected_grad, self.TV_grad],
    #                                feed_dict={self.input_image: x, self.data_term: true,
    #                                           self.ground_truth: image})
    #         if tensorboard:
    #             writer.add_summary(summary, k)
    #         x = self.update(x, uncor_grad, TV_grad, lam=lam, step_size=step_size, positivity=positivity)
    #     if tensorboard:
    #         writer.flush()
    #         writer.close()
